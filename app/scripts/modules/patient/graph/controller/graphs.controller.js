...
.controller('graphController',
  ...
  function($scope, $state, patientDashBoardService, StorageService, dateService, graphUtil, patientService, UserService, $stateParams, notyService, $timeout, graphService, caregiverDashBoardService, loginConstants, $location, $filter, commonsUserService) {
    ...
    $scope.addCanvasToDOM = function (){ 
      ...
      //context = document.getElementById('hmrLineCanContainer').getContext;
    };        
    $scope.isIE = function(){
    ...
    $scope.init();
      ...
      $scope.drawHMRLineGraph = function() {
        ...
        $timeout(function() {
          ...
          html1 = (element1) ? element1.innerHTML: "",          
          doc;
          $scope.initPdfMetaData(html1, "hmrLine");
          },500);
        });
    }
...
//end
    ...
    $scope.initPdfMetaData = function (strHtml, graphType){
      ...
      switch(graphType) {
        ...
        /*$q.all(graphRequests).then(function(data) {        
          if(data){ 
            var hmrLineData = (hmrLineIndex !== -1) ? data[hmrLineIndex] : null;
            var hmrBarData = (hmrBarIndex !== -1) ? data[hmrBarIndex] : null;
            var complianceData = (complianceIndex !== -1) ? data[complianceIndex] : null;
            if(hmrLineData){
              $scope.getSVGForHMRLineGraph(hmrLineData);
            }
            if(hmrBarData){
              $scope.getSVGForHMRBarGraph(hmrBarData);
            }
            if(complianceData){
              $scope.getSVGForComplianceGraph(complianceData);
            }
          }
        });
*/
    ...
    $scope.downloadPDFFile = function (){        
      ...
      if($("#hmrLineDiv").find("svg").length > 0){
        ...
        var img = document.getElementById("hmrLineCanvas").toDataURL("image/PNG");
      ...
      if($("#hmrBarDiv").find("svg").length > 0){
        ...
        var img = document.getElementById("hmrBarCanvas").toDataURL("image/PNG");
      ...
      if($("#complianceDiv").find("svg").length > 0){
       /* if(isIEBrowser){
        ...
        var img = document.getElementById("complianceCanvas").toDataURL("image/PNG");
    ...
    $scope.drawCanvas = function(id, html){      
      ...
      if($scope.isIE()>0){
        context = document.getElementById(id).getContext;

            var canvas = document.getElementById(id);               
            //var ctx = canvas.getContext('2d');
            if (context) canvas.getContext = context;
            canvg(canvas, html.trim());

           /* canvg(canvas, html.trim(), { renderCallback: function () {
